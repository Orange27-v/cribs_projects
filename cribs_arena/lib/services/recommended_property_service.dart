...
class RecommendedPropertyService {
  ...
  RecommendedPropertyService({http.Client? client})
  ...
  Future<List<Property>> getRecommendedProperties(
    double latitude,
    double longitude, {
    double radius = 50.0, // Default 50km radius
    bool forceRefresh = false,
  }) async {
    try {
      final response = await _get(
        '/properties/recommended?latitude=$latitude&longitude=$longitude&radius=$radius',
      );
      final properties = _decodeJson<List<Property>>(
        ...
        (json) {
          ...
          throw NetworkException(
            'Unexpected JSON format for recommended properties',
            NetworkErrorType.serverError,
          );
        },
      );
