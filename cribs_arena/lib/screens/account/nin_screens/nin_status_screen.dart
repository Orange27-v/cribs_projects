...
import 'package:cribs_arena/services/nin_service.dart';
import 'package:cribs_arena/screens/account/widgets/pending_status_widget.dart';
import 'package:cribs_arena/screens/account/widgets/nin_status_widget.dart';
...
class NinStatusScreen extends StatefulWidget {
  final String verificationId;

  const NinStatusScreen({super.key, required this.verificationId});

  @override
  State<NinStatusScreen> createState() => _NinStatusScreenState();
}

class _NinStatusScreenState extends State<NinStatusScreen> {
  ...
  Future<void> _refreshUserProfile() async {
    try {
      final userData = await _userAuthService.fetchUserData();
      if (mounted) {
    ...
    try {
      await _pusher.init(
          ...
          onError: (message, code, e) {
            // Pusher error - continue with polling
          });
  ...
  Future<void> _fetchInitialStatus() async {
    try {
      final ninService = NinService(); // Renamed from _ninService
      final status = await ninService.getNinStatus(widget.verificationId);
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: const PrimaryAppBar(
        title: Text('NIN VERIFICATION STATUS'),
      ),
      body: Padding(
        padding: kPaddingAll16,
        child: _isLoading || _verificationStatus == null
            ? PendingStatusWidget(
                message: _errorMessage ?? 'Checking NIN verification status...')
            : NinStatusWidget(
