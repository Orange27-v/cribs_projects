...
class _MyFeedScreenState extends State<MyFeedScreen> {
  ...
  Future<void> _fetchNewListings(double latitude, double longitude) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          if (_newListings.isEmpty) {
            _newListingsError = 'Failed to load new listings';
          }
          _loadingNewListings = false;
        });
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "Recommended agents and properties based on your preferences",
            style: GoogleFonts.roboto(
  ...
  Widget _appointmentsCard() => StreamBuilder<int>(
  ...
  Widget _recommendedPropertiesWidget() {
    ...
    if (_recommendedPropertiesError != null) {
      return SizedBox(
        ...
        child: Center(
          child: Text(
            _recommendedPropertiesError ?? 'Error loading properties',
            style: const TextStyle(color: kGrey600),
          ),
        ),
      );
  ...
  Widget _newListingsWidget() {
    ...
    if (_newListingsError != null) {
      return SizedBox(
        ...
        child: Center(
          child: Text(
            _newListingsError ?? 'Error loading listings',
            style: const TextStyle(color: kGrey600),
          ),
        ),
      );
