    ...
    return OutlinedButton(
      ...
      style: OutlinedButton.styleFrom(
        ...
        side: BorderSide(
            ...
            width: borderWidth ?? kStrokeWidth1_5),
        minimumSize: const Size.fromHeight(kSizedBoxH48),
    ...
    return Column(
      children: [
        Image.asset(
          ...
          height: kSizedBoxH150,
        ),
        const SizedBox(height: kSizedBoxH0),
        Image.asset(
...
/// A modern loading dialog with consistent branding
/// Displayed as an overlay during long-running async operations
...
/// Circular Refresh Indicator Widget
...
// CustomLoadingSpinner was removed in favor of CustomLoadingIndicator above.
...
class TransactionListItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isIncoming = transaction.type.toString().contains('incoming');
    final status = transaction.status?.toString().toLowerCase() ?? 'success';

    // Modern gradient colors
    final Color iconBgStart = isIncoming
        ? const Color(0xFF10B981) // Emerald
        : const Color(0xFFEF4444); // Red
    final Color iconBgEnd = isIncoming
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    '${isIncoming ? '+' : '-'}${NumberFormat.currency(symbol: nairaSymbol, decimalDigits: 2).format(transaction.amount)}',
                    style: GoogleFonts.roboto(
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
                      color: amountColor,
...
/// Helper method to resolve image URLs consistently across the app
...
/// Helper to get an image provider (Network or Asset)
