...
import 'agent_profile_details_screen.dart';
...
class _TermAgreementScreenState extends State<TermAgreementScreen>
  ...
  Future<Map<String, String>> _loadLegalDocuments() async {
    try {
      final termsResponse =
          await _legalService.fetchLegalDocument('agent_terms_of_service');
      final privacyResponse =
          await _legalService.fetchLegalDocument('agent_privacy_policy');
    ...
    try {
      ...
      if (mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (context) => const AgentProfileDetailsScreen()),
        );
      }
    } catch (e) {
      if (mounted) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          AnimatedContainer(
            duration: const Duration(milliseconds: 300),
            height: _showScrollButton ? 48 : 0,
            child: _showScrollButton
                ? OutlinedActionButton(
                    text: _isAtBottom ? 'Scroll to top' : 'Scroll to bottom',
