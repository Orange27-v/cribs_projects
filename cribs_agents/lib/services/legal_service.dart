...
import 'package:cribs_agents/constants.dart'; // Added import

class LegalService {
  final String _baseUrl = kBaseUrl; // Replaced hardcoded URL with kBaseUrl
  final String _agentBaseUrl = kAgentBaseUrl; // Agent base URL
  ...
  Future<Map<String, dynamic>> fetchLegalDocument(String type) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load legal document: ${response.reasonPhrase}');
    }
  }
  ...
  Future<void> agreeToTerms(String version) async {
    ...
    final uri = Uri.parse('$_agentBaseUrl/agree-to-terms'); // Use kAgentBaseUrl
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to record agent agreement: ${response.reasonPhrase}');
    }
  }
}
