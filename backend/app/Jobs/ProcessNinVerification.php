...
class ProcessNinVerification implements ShouldQueue
{
    ...
    {
        try {
            $qoreidResponse = $verificationService->verifyNin($this->verification->value, $this->requestData);
            ...
            if (isset($qoreidResponse['status']['state']) && $qoreidResponse['status']['state'] === 'verified') {
                ...
                isset($qoreidResponse['summary']['nin_check']['status']) &&
                in_array($qoreidResponse['summary']['nin_check']['status'], ['EXACT_MATCH', 'VERIFIED'])
            ) {
                $status = 'verified';
            }
            ...
            Log::error('NIN verification job failed for verification_id: ' . $this->verification->verification_id, [
                'error' => $e->getMessage(),
