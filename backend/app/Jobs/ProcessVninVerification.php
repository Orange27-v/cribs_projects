...
class ProcessVninVerification implements ShouldQueue
{
    ...
    {
        try {
            $qoreidResponse = $verificationService->verifyVnin($this->verification->value, $this->requestData);
            ...
            if (isset($qoreidResponse['status']['state']) && $qoreidResponse['status']['state'] === 'verified') {
                ...
                isset($qoreidResponse['summary']['v_nin_check']['status']) &&
                in_array($qoreidResponse['summary']['v_nin_check']['status'], ['EXACT_MATCH', 'VERIFIED'])
            ) {
                $status = 'verified';
            }
            ...
            Log::info('vNIN verification completed', [
                'verification_id' => $this->verification->verification_id,
                'status' => $status,
            ]);

        } catch (\Exception $e) {
            Log::error('vNIN verification job failed for verification_id: ' . $this->verification->verification_id, [
                'error' => $e->getMessage(),
